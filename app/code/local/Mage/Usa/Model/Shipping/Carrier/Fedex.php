    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    protected function _prepareRateResponse($response)
    {
        ...
        if (empty($priceArr)) {
          ...
          //  Mage::log($error."err",null,'fed.log');
            $error->setCarrier($this->_code);
                 // Mage::log($this->_code."code",null,'fed.log');
            ...
            foreach ($priceArr as $method=>$price) {
                //Mage::log($method,null,'fed.log');
                // Mage::log($price,null,'fed.log');
                ...
                //changes for double shipping rate
                //by pooja 28june2015
                //multiply orginal price by 2                                 
                if($method != "FEDEX_GROUND" && $method != 'GROUND_HOME_DELIVERY'){               
                	$rate->setPrice($price*2);
	            }
	            else{
	                $rate->setPrice($price);
	            }
            //end of changes for double shipping rates
                $result->append($rate);
            }
        }
        return $result;
